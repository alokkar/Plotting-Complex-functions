...
nzline=np.cos(zline)*(np.exp(Z)+np.exp(-Z)/2)
NZ=(-1)*np.sin(zline)*(np.exp(Z)-np.exp(-Z)/2)

color_dimension = NZ # change to desired fourth dimension
...
color_dimension2 = nzline # change to desired fourth dimension
...
surf1=ax1.plot_surface(X,Y,nzline,facecolors=fcolors,linewidth=0)
# surf2=ax1.plot_surface(X,Y,(-1)*zline,facecolors=fcolors1,linewidth=0)
surf3=ax2.plot_surface(X,Y,NZ,facecolors=fcolors2,linewidth=0)
